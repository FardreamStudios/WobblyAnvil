          ...
          `}</style>
                                        ...
                                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, width: "100%", maxWidth: 400 }}>
                                            {wipWeapon ? (
                                                ...
                                                <><div style={{ fontSize: 16, letterSpacing: 3, color: "#f59e0b", fontWeight: "bold" }}>FORGE READY</div><SectionLabel>{sessCost + "HR/SESSION"}</SectionLabel>
                                                    <button data-fairy-target="btn_forge_start" onClick={input.beginForge.redirectToRest ? waitHour : input.beginForge.disabled ? null : function(e) { e.stopPropagation(); props.onBeginForge(); }} disabled={input.beginForge.disabled} style={{ background: "#2a1f0a", border: "2px solid #f59e0b", borderRadius: 8, color: "#f59e0b", padding: "14px 40px", fontSize: 18, cursor: "pointer", letterSpacing: 2, textTransform: "uppercase", fontFamily: "monospace", fontWeight: "bold", position: "relative", pointerEvents: tutorialHighlight ? "none" : "auto" }}><span style={{ opacity: input.beginForge.redirectToRest ? 0.65 : 1 }}>Begin Forging</span>{input.beginForge.redirectToRest && <span className="blink-slow" style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, pointerEvents: "none", zIndex: 2 }}>{"\u23F3"}</span>}</button></>
                                            )}
                                        </div>
                                    </div>
                                )}
                ...
                <GameFooter>
                    <div data-fairy-target="btn_area" style={{ display: "flex", gap: 5, flexShrink: 0, height: 80, pointerEvents: tutorialHighlight ? "none" : "auto" }}>
                        {[[IC.sleep, "Sleep", function() { sfx.click(); sleep(); }, input.sleep.disabled, input.sleep.redirectToRest, "btn_sleep"], [IC.rest, "Rest", waitHour, input.rest.disabled, input.rest.redirectToRest, "btn_rest"], [IC.promote, "Promote", promote, input.promote.disabled, input.promote.redirectToRest, null], [IC.scavenge, "Scavenge", scavenge, input.scavenge.disabled, input.scavenge.redirectToRest, "btn_scavenge"]].map(function(b) {
                            var iconSrc = b[0], label = b[1], fn = b[2], dis = b[3], needRest = b[4], fairyId = b[5];
                            var finalFn = needRest ? waitHour : fn;
                            var iconFilter = dis ? "brightness(0.3)" : "drop-shadow(0 0 1px #000) drop-shadow(0 0 1px #000)";
                            var btnEl = (<button key={label} onClick={dis ? null : finalFn} disabled={dis} style={{ background: dis ? "#0a0704" : "#141009", border: "1px solid " + (dis ? "#1a1209" : "#2a1f0a"), borderRadius: 7, color: dis ? "#2a1f0a" : "#8a7a64", cursor: dis ? "not-allowed" : "pointer", fontFamily: "monospace", fontWeight: "bold", fontSize: 11, letterSpacing: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, height: "100%", width: 72, padding: 0, position: "relative" }}>
                                <span style={{ opacity: needRest ? 0.65 : 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}><img src={iconSrc} alt={label} style={{ width: 28, height: 28, imageRendering: "pixelated", filter: iconFilter }} /><span>{label.toUpperCase()}</span></span>
