          ...
          `}</style>
                                ...
                                {phase === PHASES.SESS_RESULT && sessResult && (
                                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, width: "100%", pointerEvents: tutorialHighlight ? "none" : "auto" }}>
                                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, width: "60%", marginTop: 4 }}>
                                            ...
                                            <button data-fairy-target="btn_scrap" onClick={function(e) { e.stopPropagation(); sfx.click(); scrapWeapon(); }} style={{ background: "#141009", border: "2px solid #3d2e0f", borderRadius: 8, color: "#8a7a64", padding: "8px 4px", fontSize: 11, cursor: "pointer", letterSpacing: 1, textTransform: "uppercase", fontFamily: "monospace", fontWeight: "bold" }}>Scrap</button>
                                            <button data-fairy-target="btn_leave" onClick={function(e) { e.stopPropagation(); sfx.click(); takeBreak(); }} style={{ background: "#141009", border: "2px solid #60a5fa", borderRadius: 8, color: "#60a5fa", padding: "8px 4px", fontSize: 11, cursor: "pointer", letterSpacing: 1, textTransform: "uppercase", fontFamily: "monospace", fontWeight: "bold", gridColumn: "span 2" }}>Leave Forging</button>
                                        </div>
                                    </div>
                                )}
                            </div>
                ...
                <GameFooter>
                    ...
                    <div style={{ flex: 1, display: "flex", gap: 8, alignItems: "center", justifyContent: "flex-start" }}>
                        <div data-fairy-target="btn_shop"><ActionBtn onClick={function() { sfx.click(); setShowShop(function(s) { return !s; }); }} disabled={input.shop.disabled} style={{ height: 80, padding: "0 18px", fontSize: 14, flexShrink: 0 }}>{"\uD83D\uDED2"} Shop</ActionBtn></div>
                        <div data-fairy-target="btn_mats"><button onClick={input.mats.disabled ? null : function() { sfx.click(); setShowMaterials(function(s) { return !s; }); }} disabled={input.mats.disabled} style={{ height: 80, padding: "0 14px", fontSize: 12, flexShrink: 0, background: "#0f0b06", border: "1px solid " + (input.mats.disabled ? "#1a1209" : "#3d2e0f"), borderRadius: 8, color: input.mats.disabled ? "#2a1f0a" : "#5a4a38", cursor: input.mats.disabled ? "not-allowed" : "pointer", letterSpacing: 2, textTransform: "uppercase", fontFamily: "monospace", fontWeight: "bold" }}>{"\u2697"} Mats</button></div>
