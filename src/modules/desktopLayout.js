...
function DesktopLayout(props) {
    ...
    var tutorialHighlight = vfx.tutorialHighlight;
    var fxRef = vfx.fxRef;
    ...
    var heatModifierScale = forgeVM.heatModifierScale;
        ...
        <>
          ...
          `}</style>
                ...
                <GameCenter>
                    ...
                    {/* FORGE AREA */}
                    {!activeCustomer && (
                        <div data-fairy-target="scene" onMouseDown={isQTEActive && !tutorialHighlight ? onForgeClick : null} style={{ background: "#0f0b06", border: "1px solid #3d2e0f", borderRadius: 10, padding: "12px", display: "flex", flexDirection: "column", alignItems: "center", gap: 4, position: "relative", cursor: isQTEActive && !tutorialHighlight ? "pointer" : "default", flex: 1, minHeight: 280, overflow: "hidden" }}>
                            {/* SCENE */}
                            ...
                            <div style={{ position: "relative", zIndex: 10, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "flex-end", gap: 4, width: "100%", flex: 1 }}>
                                        ...
                                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 14, width: "100%", maxWidth: 400 }}>
                                            {wipWeapon ? (
                                                <div style={{ width: "100%", background: "#0a0704", border: "2px solid #60a5fa", borderRadius: 12, padding: "18px 20px" }}>
                                                    ...
                                                    <div style={{ display: "flex", gap: 8, pointerEvents: tutorialHighlight ? "none" : "auto" }}>
                                                    ...
                                                    {input.noStamina && <div style={{ fontSize: 10, color: "#fb923c", letterSpacing: 1, textAlign: "center", marginTop: 6 }}>NO STAMINA — REST BEFORE RESUMING</div>}
                                                </div>
                                            ) : (
                                                <><div style={{ fontSize: 16, letterSpacing: 3, color: "#f59e0b", fontWeight: "bold" }}>FORGE READY</div><SectionLabel>{sessCost + "HR/SESSION"}</SectionLabel>
                                                    <button data-fairy-target="btn_forge_start" onClick={input.beginForge.disabled ? null : function(e) { e.stopPropagation(); props.onBeginForge(); }} disabled={input.beginForge.disabled} style={{ background: "#2a1f0a", border: "2px solid #f59e0b", borderRadius: 8, color: "#f59e0b", padding: "14px 40px", fontSize: 18, cursor: "pointer", letterSpacing: 2, textTransform: "uppercase", fontFamily: "monospace", fontWeight: "bold", position: "relative", pointerEvents: tutorialHighlight ? "none" : "auto" }}>Begin Forging</button></>
                                            )}
                                        </div>
                                    </div>
                                )}
                                ...
                                {phase === PHASES.SESS_RESULT && sessResult && (
                                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, width: "100%", pointerEvents: tutorialHighlight ? "none" : "auto" }}>
                                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, width: "60%", marginTop: 4 }}>
                                            {(function() { var s = sessResult.ns; var dis = input.forge.disabled; var morphRest = input.noStamina && !input.rest.disabled; var borderCol = morphRest ? "#f59e0b" : dis ? "#2a1f0a" : s >= STRESS_MAX ? "#ef4444" : s >= STRESS_MAX - 1 ? "#fb923c" : "#f59e0b"; var textCol = morphRest ? "#f59e0b" : dis ? "#4a3c2c" : s >= STRESS_MAX ? "#ef4444" : s >= STRESS_MAX - 1 ? "#fb923c" : "#f59e0b"; var bg = morphRest ? "#2a1f0a" : dis ? "#0a0704" : s >= STRESS_MAX ? "#1a0505" : s >= STRESS_MAX - 1 ? "#1a0e05" : "#2a1f0a"; return <button data-fairy-target="btn_forge_again" onClick={morphRest ? function(e) { e.stopPropagation(); sfx.click(); waitHour(); } : dis ? null : function(e) { e.stopPropagation(); sfx.click(); attemptForge(); }} disabled={!morphRest && dis} style={{ background: bg, border: "2px solid " + borderCol, borderRadius: 8, color: textCol, padding: "8px", fontSize: 11, cursor: (!morphRest && dis) ? "not-allowed" : "pointer", letterSpacing: 1, textTransform: "uppercase", fontFamily: "monospace", fontWeight: "bold", position: "relative", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}><img src={morphRest ? IC.rest : IC.forge} alt={morphRest ? "Rest" : "Forge"} style={{ width: 22, height: 22, imageRendering: "pixelated", filter: (!morphRest && dis) ? "brightness(0.3)" : "none" }} /><span>{morphRest ? "REST" : "FORGE"}</span>{!morphRest && s >= STRESS_MAX - 1 && <span className="blink-fast" style={{ fontSize: 10, color: s >= STRESS_MAX ? "#ef4444" : "#fb923c", position: "absolute", right: 6, top: "50%", transform: "translateY(-50%)" }}>{s >= STRESS_MAX ? Math.round(BALANCE.shatterChanceMax * 100) + "%" : Math.round(BALANCE.shatterChanceHigh * 100) + "%"} BREAK</span>}</button>; })()}
                                            <button data-fairy-target="btn_normalize" disabled={input.normalize.disabled} onClick={function(e) { e.stopPropagation(); sfx.click(); doNormalize(); }} style={{ background: input.normalize.disabled ? "#0a0704" : "#0a1a2a", border: "2px solid " + (input.normalize.disabled ? "#2a1f0a" : "#60a5fa"), borderRadius: 8, color: input.normalize.disabled ? "#4a3c2c" : "#60a5fa", padding: "8px 4px", fontSize: 11, cursor: input.normalize.disabled ? "not-allowed" : "pointer", letterSpacing: 1, textTransform: "uppercase", fontFamily: "monospace", fontWeight: "bold", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}><img src={IC.normalize} alt="Normalize" style={{ width: 22, height: 22, imageRendering: "pixelated", filter: input.normalize.disabled ? "brightness(0.3)" : "none" }} />Normalize</button>
                                            <button data-fairy-target="btn_quench" onClick={input.quench.disabled ? null : function(e) { e.stopPropagation(); sfx.click(); setPhase(PHASES.QUENCH); }} disabled={input.quench.disabled} style={{ background: input.quench.disabled ? "#0a0704" : "#2a1f0a", border: "2px solid " + (input.quench.disabled ? "#2a1f0a" : "#f59e0b"), borderRadius: 8, color: input.quench.disabled ? "#4a3c2c" : "#f59e0b", padding: "8px 4px", fontSize: 11, cursor: input.quench.disabled ? "not-allowed" : "pointer", letterSpacing: 1, textTransform: "uppercase", fontFamily: "monospace", fontWeight: "bold", position: "relative", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}>
                                                <img src={IC.quench} alt="Quench" style={{ width: 22, height: 22, imageRendering: "pixelated", filter: input.quench.disabled ? "brightness(0.3)" : "none" }} />
                                                <span>Quench</span>
                                            </button>
                                            <button data-fairy-target="btn_scrap" onClick={function(e) { e.stopPropagation(); sfx.click(); scrapWeapon(); }} style={{ background: "#141009", border: "2px solid #3d2e0f", borderRadius: 8, color: "#8a7a64", padding: "8px 4px", fontSize: 11, cursor: "pointer", letterSpacing: 1, textTransform: "uppercase", fontFamily: "monospace", fontWeight: "bold", display: "flex", flexDirection: "column", alignItems: "center", gap: 2 }}><img src={IC.scrap} alt="Scrap" style={{ width: 22, height: 22, imageRendering: "pixelated" }} />Scrap</button>
                                            <button data-fairy-target="btn_leave" onClick={function(e) { e.stopPropagation(); sfx.click(); takeBreak(); }} style={{ background: "#141009", border: "2px solid #60a5fa", borderRadius: 8, color: "#60a5fa", padding: "8px 4px", fontSize: 11, cursor: "pointer", letterSpacing: 1, textTransform: "uppercase", fontFamily: "monospace", fontWeight: "bold", gridColumn: "span 2", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}><img src={IC.leave} alt="Leave" style={{ width: 22, height: 22, imageRendering: "pixelated" }} />Leave Forging</button>
                                        </div>
                                    </div>
                                )}
                            </div>
                ...
                {/* BOTTOM BAR */}
                <GameFooter>
                    <div data-fairy-target="btn_area" style={{ display: "flex", gap: 5, flexShrink: 0, height: 80, pointerEvents: tutorialHighlight ? "none" : "auto" }}>
                        {[[IC.sleep, "Sleep", function() { sfx.click(); sleep(); }, input.sleep.disabled, "btn_sleep"], [IC.rest, "Rest", waitHour, input.rest.disabled, "btn_rest"], [IC.promote, "Promote", promote, input.promote.disabled, null], [IC.scavenge, "Scavenge", scavenge, input.scavenge.disabled, "btn_scavenge"]].map(function(b) {
                            var iconSrc = b[0], label = b[1], fn = b[2], dis = b[3], fairyId = b[4];
                            var iconFilter = dis ? "brightness(0.3)" : "drop-shadow(0 0 1px #000) drop-shadow(0 0 1px #000)";
                            var btnEl = (<button key={label} onClick={dis ? null : fn} disabled={dis} style={{ background: dis ? "#0a0704" : "#141009", border: "1px solid " + (dis ? "#1a1209" : "#2a1f0a"), borderRadius: 7, color: dis ? "#2a1f0a" : "#8a7a64", cursor: dis ? "not-allowed" : "pointer", fontFamily: "monospace", fontWeight: "bold", fontSize: 11, letterSpacing: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, height: "100%", width: 72, padding: 0, position: "relative" }}>
                                <span style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}><img src={iconSrc} alt={label} style={{ width: 28, height: 28, imageRendering: "pixelated", filter: iconFilter }} /><span>{label.toUpperCase()}</span></span>
                            </button>);
                            return fairyId ? <div key={label} data-fairy-target={fairyId}>{btnEl}</div> : btnEl;
                        })}
                    </div>
                    ...
                    <div style={{ flex: 1, display: "flex", gap: 8, alignItems: "center", justifyContent: "flex-start" }}>
                        <div data-fairy-target="btn_shop"><ActionBtn onClick={function() { sfx.click(); setShowShop(function(s) { return !s; }); }} disabled={input.shop.disabled} style={{ height: 80, padding: "0 18px", fontSize: 14, flexShrink: 0 }}><img src={IC.shop} alt="Shop" style={{ width: 28, height: 28, imageRendering: "pixelated", filter: input.shop.disabled ? "brightness(0.3)" : "drop-shadow(0 0 1px #000) drop-shadow(0 0 1px #000)" }} /> Shop</ActionBtn></div>
                        <div data-fairy-target="btn_mats"><button onClick={input.mats.disabled ? null : function() { sfx.click(); setShowMaterials(function(s) { return !s; }); }} disabled={input.mats.disabled} style={{ height: 80, padding: "0 14px", fontSize: 12, flexShrink: 0, background: "#0f0b06", border: "1px solid " + (input.mats.disabled ? "#1a1209" : "#3d2e0f"), borderRadius: 8, color: input.mats.disabled ? "#2a1f0a" : "#5a4a38", cursor: input.mats.disabled ? "not-allowed" : "pointer", letterSpacing: 2, textTransform: "uppercase", fontFamily: "monospace", fontWeight: "bold", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3 }}><img src={IC.bag} alt="Mats" style={{ width: 28, height: 28, imageRendering: "pixelated", filter: input.mats.disabled ? "brightness(0.3)" : "drop-shadow(0 0 1px #000) drop-shadow(0 0 1px #000)" }} /> Mats</button></div>
